...
describe('GameOverlay', () => {
  ...
  it('uses project-local archer assets for the character selection and returns through selection without starting a run', () => {
    ...
    expect(screen.getByTestId('character-detail-evolution-image-wind-cut').getAttribute('data-scene-asset-logical-url')).toBe(getArcherSkillIconAssetUrl('wind-cut'))
    expect(undiscoveredEvolution.tagName).toBe('DIV')
  ...
  it('shows only confirmed talent point balance and settlement records in hunter home', () => {
    ...
    expect(screen.getByTestId('meta-talent-node-icon-meta_common_01').getAttribute('data-scene-asset-logical-url')).toContain(encodeURIComponent('契约记忆.png'))
    expect(screen.getByTestId('meta-talent-node-icon-meta_common_02').getAttribute('data-scene-asset-logical-url')).toContain(encodeURIComponent('初始重绑.png'))
    fireEvent.focus(screen.getByTestId('meta-talent-node-meta_common_01'))
    expect(screen.getByTestId('meta-talent-tooltip-icon-image-meta_common_01').getAttribute('data-scene-asset-logical-url')).toContain(encodeURIComponent('契约记忆.png'))
    expect(screen.queryByTestId('hunter-home-run-talent-panel')).toBeNull()
