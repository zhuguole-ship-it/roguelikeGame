...
const inferBuildTag = (id: string, kind: SkillBehaviorKind): SkillBuildTag => {
  if (id === 'spiral-break' || id === 'cross-cut' || id === 'blood-scent') {
    return 'pierce'
  }
...
export const LV5_QUALITATIVE_TEXT: Record<string, string> = {
  ...
  'heavy-snipe': '旧存档载入时迁移为螺旋破空，保留等级与槽位',
  ...
  'dawn-bolt': '旧存档载入时迁移为交叉切射，保留对应进化',
  ...
  'weakness-trace': '旧存档载入时迁移为血嗅追猎，保留对应进化',
  ...
  'spiral-break': '单支螺旋追击箭按命中预算持续转向',
  'cross-cut': '两支反向追击箭同步抵达时结算交叉切击',
  'blood-scent': '每次转向优先真实可斩杀目标',
...
export const ARCHER_ACTIVE_SKILLS: ActiveSkillDefinition[] = [
  ...
  // Legacy icon/name registry only. Engine migration maps this ID before any
  // reward, cast, build, or modifier consumer can observe it.
  skill('heavy-snipe', '重矢狙击', '旧存档迁移到螺旋破空。', 'beam', { damage: 7, cooldown: 4.8, range: 520, pierce: 1, speed: 340 }, { damage: 1.6, cooldown: -0.18, range: 35, pierce: 0.4 }),
  ...
  skill('dawn-bolt', '破晓圣矢', '旧存档迁移到交叉切射。', 'beam', { damage: 6, cooldown: 4.6, range: 520, pierce: 1, color: '#fde68a' }, { damage: 1.1, cooldown: -0.12, range: 32, pierce: 0.2 }),
  ...
  skill('spiral-break', '螺旋破空', '前进中的螺旋追击箭。', 'orbit', { damage: 3.5, cooldown: 4.6, projectileCount: 10, spread: 0, range: 260, color: '#a78bfa' }, { damage: 0.7, cooldown: -0.1, projectileCount: 2 }),
  ...
  skill('cross-cut', '交叉切射', '两支反向螺旋追击箭同步交叉。', 'orbit', { damage: 4.1, cooldown: 4.1, projectileCount: 6, spread: 0, range: 340 }, { damage: 0.8, cooldown: -0.1, projectileCount: 1, spread: 0 }),
  ...
  skill('weakness-trace', '弱点追索', '旧存档迁移到血嗅追猎。', 'projectile', { damage: 5.2, cooldown: 3.9, pierce: 1, range: 360, color: '#ddd6fe' }, { damage: 0.9, cooldown: -0.11, pierce: 0.25 }),
  ...
  skill('blood-scent', '血嗅追猎', '螺旋追击并优先真实可斩杀目标。', 'orbit', { damage: 4.3, cooldown: 3.7, projectileCount: 4, spread: 0, range: 360, color: '#fda4af' }, { damage: 0.8, cooldown: -0.11, projectileCount: 1 }),
